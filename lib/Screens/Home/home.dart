...
class _HomePageState extends State<HomePage> {
  ...
  Widget checkVersion() {
    if (!checked && Theme.of(context).platform == TargetPlatform.android) {
      ...
      final List lastLogin = now.toUtc().add(const Duration(hours: 5, minutes: 30)).toString().split('.')
        ..removeLast()
        ..join('.');
      updateUserDetails('lastLogin', '${lastLogin[0]} IST');
      final String offset = now.timeZoneOffset.toString().replaceAll('.000000', '');

      updateUserDetails('timeZone', 'Zone: ${now.timeZoneName}, Offset: $offset');
                      ...
                      SliverAppBar(
                        ...
                        flexibleSpace: FlexibleSpaceBar(
                          ...
                          background: ShaderMask(
                            ...
                            child: Image(alignment: Alignment.topCenter, image: AssetImage(Theme.of(context).brightness == Brightness.dark ? 'assets/header-dark.jpg' : 'assets/header.jpg')),
                          ),
                        ),
                      ),
                      SliverList(
                        delegate: SliverChildListDelegate(
                          [
                            ...
                            ListTile(
                              ...
                              onTap: () {
                                Navigator.pop(context);
                                Navigator.push(context, MaterialPageRoute(builder: (context) => const DownloadedSongs(type: 'all')));
                              },
                            ),
                            ListTile(
                              ...
                              onTap: () {
                                Navigator.pop(context);
                                Navigator.push(context, MaterialPageRoute(builder: (context) => SettingPage(callback: callback)));
                              },
                            ),
                            ListTile(
                ...
                Expanded(
                  child: PageView(
                    ...
                    children: [
                                    ...
                                    builder: (BuildContext context, BoxConstraints constraints) {
                                      return FlexibleSpaceBar(
                                        ...
                                        background: GestureDetector(
                                          onTap: () async {
                                            await TextInputDialog().showTextInputDialog(context, 'Name', name, TextInputType.name, (value) {
                                              ...
                                              Row(
                                                children: [
                                                  Padding(
                                                    ...
                                                    child: Text(
                                                      'Hi There,',
                                                      style: TextStyle(letterSpacing: 2, color: Theme.of(context).accentColor, fontSize: 30, fontWeight: FontWeight.bold),
                                                    ),
                                                  ),
                                                ],
                                              ),
                                                  ...
                                                  children: [
                                                    ValueListenableBuilder(
                                                        ...
                                                        builder: (BuildContext context, Box box, widget) {
                                                          return Text(
                                                            (box.get('name') == null || box.get('name') == '') ? 'Guest' : capitalize(box.get('name').split(' ')[0].toString()),
                                                            style: const TextStyle(letterSpacing: 2, fontSize: 20, fontWeight: FontWeight.w500),
                                                          );
                                                        }),
                                                  ],
                                                ),
                                        ...
                                        builder: (context, child) {
                                          return AnimatedContainer(
                                            width: (!_scrollController.hasClients ||
                                                ...
                                                : max(MediaQuery.of(context).size.width - _scrollController.offset.roundToDouble(), MediaQuery.of(context).size.width - 75),
                      ...
                      TopCharts(
                        region: CountryCodes().countryCodes[Hive.box('settings').get('region', defaultValue: 'India')].toString(),
                      ),
                      const YouTube(),
                      LibraryPage(),
                    ],
                  ...
                  child: ValueListenableBuilder(
                      ...
                      builder: (BuildContext context, int indexValue, Widget? child) {
                        return AnimatedContainer(
                          ...
                          height: 60 * (MediaQuery.of(context).size.height - value) / (MediaQuery.of(context).size.height - 76),
